...
A bot sends a POST request directly to the OTP request endpoint with a valid JSON body and a forged, invalid, expired, or missing Turnstile token. The system rejects the request with a generic error before any email is generated or sent. In non-production environments, missing tokens fail open (subject to all other rate limits) to support automated testing.
...
**Independent Test**: Can be tested by sending a curl/HTTP request to the OTP endpoint with an invalid Turnstile token and verifying it is rejected with a generic 400 error. A second test with no token in production should confirm the request is also rejected.

**Acceptance Scenarios**:

1. **Given** a request to the OTP endpoint has no Turnstile token in production, **When** the server processes it, **Then** the system rejects the request with a generic error per FR-019. In non-production environments, the system fails open and processes the request subject to all other rate limits.
...
3. **Given** a bot attempts to probe the admin-check endpoint without a Turnstile token in production, **When** the server processes the request, **Then** the system rejects the request with a generic error per FR-019. A global rate limit on the check-admin endpoint also caps total requests across all callers to prevent distributed enumeration.
...
- What happens when the Turnstile verification service is temporarily unreachable from the backend? The system uses a circuit-breaker pattern: fails open for the first few consecutive failures (up to 5 within 30 seconds) subject to all other rate limits, then fails closed until a successful verification resets the breaker (FR-013).
...
- What happens when a legitimate user's browser blocks the Turnstile script (ad blocker, strict CSP)? In production, the backend rejects requests with missing tokens. The frontend should detect widget load failures and display a message directing the user to whitelist the domain or retry. In non-production environments, the backend fails open (FR-019).
...
- **FR-009**: System MUST reject requests with invalid, expired, or missing Turnstile tokens in production. The error response MUST be a generic message (e.g., "Request could not be processed. Please try again.") that does not reveal bot detection as the rejection reason. In non-production environments, a missing token triggers fail-open behavior per FR-019.
...
- **FR-013**: If the Turnstile verification service is unreachable from the backend, the system MUST use a circuit-breaker pattern: allow requests through (fail open, subject to all other rate limits) for a limited window (up to 5 consecutive failures within 30 seconds). Once the threshold is exceeded, the system MUST reject requests (fail closed) until a successful verification resets the breaker. This prevents prolonged outages or network interference from being exploited to bypass bot protection entirely.
...
- **FR-019**: If the Turnstile widget fails to load on the client side (e.g., blocked by ad blocker, network error, or strict CSP), the frontend MUST submit the request without a Turnstile token. In **production**, the backend MUST reject requests with missing tokens to prevent trivial bypass of bot protection. The frontend SHOULD detect widget load failures and display a user-friendly message directing users to whitelist the domain or retry. In **non-production** environments, missing tokens fail open to support automated testing and local development.
...
- **SC-002**: Direct API requests to the OTP endpoint with invalid, expired, or missing Turnstile tokens are rejected 100% of the time in production.
...
- **SC-005**: When the Turnstile verification service is briefly unavailable, users can still request OTPs (protected by rate limits only) for a limited window. If the outage persists beyond the circuit-breaker threshold (5 consecutive failures within 30 seconds), requests are rejected to prevent exploitation of the outage.
...
- Q: What happens when a user's browser blocks the Turnstile script (ad blocker, strict CSP)? → A: In production, the backend rejects requests with missing tokens. The frontend detects widget load failures and displays a user-friendly message directing users to whitelist the domain or retry. In non-production environments, the backend fails open to support automated testing.
...
- "Fail open" is the appropriate degradation strategy for global rate limit storage failures, since the existing per-email/IP rate limits and suspension mechanisms provide a baseline safety net. Turnstile verification uses a circuit-breaker pattern that fails open briefly then fails closed, and missing tokens are rejected in production.
